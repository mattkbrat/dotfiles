...
return {
  ...
  config = function()
    ...
    local base_info = {
      ...
      draw = function()
        ...
        if vim.fn.isdirectory ".git" then
          ...
          if branch ~= "" then table.insert(lines, "branch: " .. branch) end
        end
    ...
    local buffers = {
      ...
      draw = function()
        ...
        for _, buf in ipairs(all_buffers) do
          if buf.name:find "^term://" ~= nil or buf.name == "" then goto continue end
    ...
    vim.api.nvim_create_autocmd({ "BufAdd", "BufDelete", "BufEnter", "TabEnter", "ModeChanged", "DiagnosticChanged" }, {
      callback = function() require("sidebar-nvim").update() end,
    })
  end,
}
